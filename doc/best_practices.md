...
> **版本**：v0.20.11 · Zig `0.17.0-dev.1422+e863bf3be` · 修订 **2026-07-31**  
...
| 2026-07-31 | **0.20.11** | 产品化 `zig build gate` / `zf release-check --strict`；模块市场 phase 1（`zf market`） |
...
**按 v0.20.11 能力写：生成器定骨架，三层定方向，路由用 actions，错误用 HttpError，拦截器 cfg 自持有，规模按 L0→L3 只换装配；合并前走 `zig build gate`；keep-alive 与 zapi 默认不翻，用文档与 ADR 锁边界。**
