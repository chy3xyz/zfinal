...
pub const CsvKit = struct {
    ...
    pub fn parseLine(line: []const u8, allocator: std.mem.Allocator) ![][]const u8 {
        ...
        errdefer { for (fields.items) |f| allocator.free(f); fields.deinit(allocator); }
    ...
    pub fn parseContent(
        ...
        errdefer { for (lines_out.items) |l| allocator.free(l); lines_out.deinit(allocator); }
...
test "CsvKit parseLine" {
    ...
    defer { for (fields) |f| allocator.free(f); allocator.free(fields); }
    ...
    defer { for (quoted) |f| allocator.free(f); allocator.free(quoted); }
    ...
    defer { for (escaped) |f| allocator.free(f); allocator.free(escaped); }
