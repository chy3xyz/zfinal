//! Skill registry export: compact skills catalog JSON or OpenAPI 3.0 document.

const std = @import("std");
const SkillRegistry = @import("skill.zig").SkillRegistry;
const Param = @import("skill.zig").Param;
...
pub fn toSkillsJson(registry: *SkillRegistry, allocator: std.mem.Allocator) ![]u8 {
    ...
    return try std.json.Stringify.valueAlloc(allocator, std.json.Value{ .object = doc }, .{});
}

/// OpenAPI 3.0.3: one `POST {base_path}/{name}` per skill. Caller frees.
...
fn collectSortedNames(registry: *SkillRegistry, allocator: std.mem.Allocator) ![]const []const u8 {
    const scratch = try allocator.alloc([]const u8, registry.count());
...
fn jsonType(t: Param.Type) []const u8 {
    return switch (t) {
        .string => "string",
        .number => "number",
        .boolean => "boolean",
