...
pub const ContextManager = struct {
    ...
    /// Index after which the tail fits within `keep_recent_tokens`.
    ...
    /// Compacts `messages` when over budget. Returned messages alias input
    /// slices — keep `messages` alive while using the result. `summary` is
    /// in/out owned text (caller frees).
    pub fn manage(
        self: ContextManager,
        allocator: std.mem.Allocator,
        messages: []const ChatMsg,
