...
zig build test             # baseline: all tests green (16 skipped without live env)
...
- `doc/index.md` — doc map for agents/humans
- `doc/zent.md` — data layer B: `zfinal.zent` as alternative / primary vs `DB`/`Model`
- `doc/outbox.md` — durable Outbox (`DbOutbox.drainOnce`) same-TX with domain writes
- `doc/bus.md` — optional Bus port (Memory / NATS / RobustMQ)
- `doc/nats.md` — NATS connector (`QueueNatsClient`, zero dep)
- `doc/robustmq.md` — RobustMQ / Kafka connector (`QueueRobustMQClient`)
- `doc/websocket.md` — WebSocket Upgrade + idle tick
- `doc/oauth2.md` — OAuth2 PKCE + optional `OAUTH2_LIVE`
