...
### Breaking
- **`DB.affectedRows` now returns `!i64`** (was `i64`): it now runs the pool's `guard()` like every other `DB` method. Add `try` at call sites — none in-tree.
- **`SessionStore.getSession` returns an owned `SessionStore.Snapshot`** (was `?*Session`). The old API handed out a pointer into the hash map after releasing the lock, so a concurrent `destroySession()` could free it mid-use (use-after-free). Call `deinit` on the snapshot; use `snapshot.get(key)` to read attributes.
- **`SessionStore.createSession` returns a caller-owned id** (was the map-key slice, which `destroySession` frees). Free it with the store's allocator.
- **`SessionStore` is now exported** from `zfinal` (`main.zig`) and therefore compiled + tested; previously it was an unimported module that never built under Zig 0.17-dev.19xx.

### Fixed
- **`Context.setHeader` is now case-insensitive** (RFC 9110), consistent with `getHeader`: `setHeader("x-foo", …)` then `setHeader("X-Foo", …)` overwrites instead of creating a duplicate response header.
- **`DB.affectedRows` returns `!i64`** and `lastInsertId` / `binlogOpen` / `binlogFetch` / `binlogClose` now run the pool's `guard()`/magic check. They previously bypassed the use-after-release detector, so reading a returned connection's driver state could race with whichever request now owns it. **Breaking**: add `try` at `affectedRows` call sites (none in-tree).
- **`MultipartParser.parse`** returns `error.InvalidMultipart` for a body truncated before the closing boundary, instead of silently treating the remaining bytes as the part content.
- **`TokenManager.validate`** is O(1) again: it no longer runs a full `cleanExpired` map scan under the mutex on every validation (that was O(n) per call). Expired tokens are reaped by a ≤60 s sweep or `purgeExpired()`, and the target token is itself checked for expiry — previously correctness depended on the pre-sweep, so an unswept expired token could have validated.
- **`AiProvider.chatStream` no longer duplicates output**: the buffered fallback now runs only when **zero** deltas were emitted (tracked by `StreamAccum.deltas_emitted`); otherwise the original transport error is surfaced instead of re-emitting the full content to a consumer that already saw partial text. The zero-delta fallback goes through the extracted `chatRetryLoop`, so `ChatOpts.max_retries` finally applies to it.
- **MCP child `stderr` is no longer discarded** (`.ignore` → `.inherit`), so server crashes/handshake failures are visible. Environment stays inherited because most MCP servers need API keys; a pipe was rejected to avoid the child blocking on an undrained buffer.
- **`crud:zent` no longer emits invalid Zig for reserved-word / non-ASCII field names.** It reused `f.name` directly at identifier positions, so an entity field named `const` or `中文` produced `pub fn createItem(self, const: []const u8, 中文: []const u8)` and `handler.zig: const const = …`. Identifier positions now go through `codegen.zigFieldName` (`@"const"`, `@"中文"`), while schema/string positions (`setFieldValue("const", …)`, `getPara("const")`, index/edge/manifest) keep the raw name. Also fixed a non-ASCII `list_by:` that emitted an invalid `listFlagBy中文` symbol.
- **HTML template engine revived**: `src/template/template.zig` had never compiled under the current Zig version (mutually-recursive inferred error sets + the removed `File.readToEndAlloc`), so it was silently orphaned and its 8 tests never ran. Fixed the error-set loop, migrated file reading, and corrected `formatValue`, which only understood `[]const u8` — Zig string literals are `*const [N:0]u8`, so **every `{{var}}` bound to a literal rendered empty**. The engine's own tests now execute and pass.
- **`http_client` timeout** (see Added) and **`Context`** fixes above are joined by a third batch: `crud:zent` **entity and module names** are now sanitized like field names (`pub const const` → `pub const @"const"`, `create中文` → `@"create中文"`, `<Mod>Store` → `@"中文Store"`), and every string position is escaped (`zigStr` for Zig literals, `jsonStr` for the manifest), so names containing `"` or `\` no longer produce invalid Zig or invalid JSON.
- **`Logger` global is race-free**: the unsynchronized `pub var global_logger` became `std.atomic.Value(?*Logger)` with static storage for the by-value install plus a lazy fallback; `initGlobalLogger` / `getLogger()` signatures are unchanged.
- **`KafkaConsumer` (RobustMQ) is thread-safe**: `state_mutex` guards maps/scalars and `transport_mutex` serializes the single Kafka socket; the two are **never nested**, so `poll` runs network I/O and user handlers lock-free and a handler can call back in without self-deadlock. Fixed in passing: `putOffset` leaked its previous key on every update, `stop()` left `transport` pointing at a closed stream, and `transportPtr()` returned `unreachable` on a concurrent stop (now `error.NotConnected`).
- **Router param cache evicts in O(1)**: `orderedRemove(0)` under the request-path mutex became a ring buffer (FIFO semantics unchanged). `deinit` also no longer leaks every cached key — it freed the slice descriptor instead of the allocation.
- **`SessionStore.getSession` no longer returns a borrowed `*Session`.** It handed out a pointer into the hash map **after releasing the lock**, so a concurrent `destroySession()` could free it mid-use (use-after-free). It now returns a caller-owned `Snapshot` (deep copy taken under the lock, `deinit` to free). Two related fixes in the same module: `createSession()` now returns a **caller-owned** id rather than the map-key slice that `destroySession` frees (callers previously held a dangling id after destroying), and the module never compiled under 0.17-dev.19xx (`std.fmt.fmtSliceHexLower` was removed), which is why the bug survived — it is now exported from `main.zig` so its tests (including a concurrent getSession-vs-destroy test) actually run.
- **`SecurityExt.validateCsrfToken` called `ctx.getSessionAttr`, which does not exist anywhere in the codebase** — a latent compile error, never caught because the function was never analyzed. It now reads `session._csrf_token` from `Context.attributes` (the attribute-based session model in `SessionExt`) and compares in constant time.

### Added
- **`zfinal.Template` / `zfinal.TemplateManager` / `zfinal.RenderEngine` are now exported.** `doc/htmx_template.md` and `examples/htmx` referenced `zfinal.TemplateManager`, but `main.zig` never exported it — a latent break masked by Zig's lazy analysis (the example declared the consts but never used them).
- **Template filters `|escape` / `|e`** (HTML-escape `& < > " '`) and explicit `|safe` / `|raw` markers. Raw interpolation stays the default to avoid silently changing existing templates; the file doc now says so.
- **`HttpClient.timeout_ms` is enforced** (it was a dead field). It is an **idle/read** timeout: an `ActivityClock` refreshed by response bytes is raced against `fetch` via `std.Io.Select.concurrent`, so a stalled upstream returns `error.Timeout` while long-lived SSE/LLM streams are not cut off at the fixed limit; `0` disables. `concurrent` (not `async`) is used because `Threaded.async` degrades to eager in-thread execution once `async_limit` is full, which would silently defeat the watchdog.
- **Randomized property test for `MultipartParser`**: 600 pseudo-random inputs (raw noise + boundary-prefixed payloads) assert no crash, panic, hang or leak under the testing allocator.

### Changed
- **CI**: new **`Unit tests (ReleaseSafe)`** job (`zig build test -Doptimize=ReleaseSafe`) catches optimization/UB-sensitive behaviour the Debug suite misses. The Debug-only `ParamQuery.toSql` tests are now compiled out via a `builtin.mode` check instead of breaking the optimized build.
- **Removed the dead `test-int` build step** and deleted `test_gen_crud.zig`: it imported app-generated `src/modules/**` paths that do not exist in the framework repo, so it could never compile. Generated-module coverage lives in `test-zfsaas` / `test-zent-shop`.
- **`release.yml` now publishes a real GitHub Release**: after the gate, a `publish` job (the only job with `contents: write`) downloads both `zf` binaries, writes `SHA256SUMS`, extracts the matching `CHANGELOG.md` section as release notes and calls `gh release create` — idempotent on re-push. See [doc/release_and_quality_gates.md](doc/release_and_quality_gates.md).
- **GitHub Actions are pinned to commit SHAs** (version kept as a trailing comment) in `ci.yml` / `release.yml`; Dependabot keeps them current. This closes the last mutable-tag supply-chain gap.
- **`doc/index.md` now indexes all 46 documentation pages** (19 were unreachable). Grouped by topic without moving files, so existing links keep working.
- **Hardcoded test counts removed from docs and badges** (they had drifted 416 → 418 → 433 within a week). The README/README_CN tests badge now shows live CI status and the guides say "all green (16 skipped without live env)"; `AGENTS.md` / `CLAUDE.md` / `doc/*` no longer pin a number.
...
### Added
- **SaaS Kit P3 admin + metered billing**: super-admin endpoints gated by `SUPER_ADMIN_EMAILS` (`/api/admin/overview|users|subscriptions` + a minimal HTML dashboard, non-admins get 403) and metered billing via a `usage_meter` table (`checkMeterQuota` / `recordMeterUsage`; todo creates enforce `TODO_MONTHLY_QUOTA`, exceeded → 402 `todo_quota_exceeded`; `GET /api/billing/usage` reports `{used,quota,period}`).
...
- **`schema.gen.sql` + annotation-derived indexes**: each module ships its verbatim `CREATE TABLE` plus `CREATE INDEX`/`CREATE UNIQUE INDEX` statements generated from `@filter` / `@sortable` / `@unique` annotations (primary keys skipped) — the declared filters are the indexes you get.
  ...
  - **SaaS Kit P2 hardening + DX**: cross-org access-control tests (update/delete/list rejection) and a fix to `updateInOrg` (dupe caller strings before assigning — `Instance.deinit` frees owned strings); committed `openapi.yaml` (generated by `zf openapi`, 25 routes); a runtime `Dockerfile` (debian-slim + sqlite, binary built on host); `quality_gate.sh` and `ci.yml` now run `zig build test-zfsaas`.
## [0.20.16] - 2026-08-03
