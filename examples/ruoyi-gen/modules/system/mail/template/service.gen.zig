...
const SystemMailTemplateModel = model.SystemMailTemplateModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemMailTemplate;
pub const Instance = SystemMailTemplateModel.Instance;

/// List all SystemMailTemplate records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemMailTemplateModel.findAll(db, allocator);
}

/// Find one SystemMailTemplate by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemMailTemplateModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemMailTemplateModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemMailTemplateModel.count(db);
}

/// Create a new SystemMailTemplate record. Validates input, returns instance with .id set.
...
/// Update SystemMailTemplate record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemMailTemplateModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemMailTemplate record by ID.
