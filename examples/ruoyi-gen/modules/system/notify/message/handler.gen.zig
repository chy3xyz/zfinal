...
/// List SystemNotifyMessage records with pagination + rate limiting.
...
/// Show SystemNotifyMessage by ID.
...
/// Create SystemNotifyMessage record (CSRF-protected).
pub fn create(ctx: *zfinal.Context) !void {
    ...
    const data: service.Data = .{
        ...
        .template_nickname = (try ctx.getPara("template_nickname")) orelse "",
        ...
        .read_status = if ((try ctx.getPara("read_status"))) |v| (std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1") or std.mem.eql(u8, v, "t")) else false,
        .read_time = (try ctx.getPara("read_time")) orelse null,
...
/// Update SystemNotifyMessage record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("read_status")) |v| item.data.read_status = std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1");
    if (try ctx.getPara("read_time")) |v| item.data.read_time = v;
...
/// Delete SystemNotifyMessage record (CSRF-protected).
...
/// Partial update SystemNotifyMessage record (CSRF-protected, PATCH).
