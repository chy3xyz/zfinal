...
const SystemNotifyMessageModel = model.SystemNotifyMessageModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemNotifyMessage;
pub const Instance = SystemNotifyMessageModel.Instance;

/// List all SystemNotifyMessage records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemNotifyMessageModel.findAll(db, allocator);
}

/// Find one SystemNotifyMessage by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemNotifyMessageModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemNotifyMessageModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemNotifyMessageModel.count(db);
}

/// Create a new SystemNotifyMessage record. Validates input, returns instance with .id set.
...
/// Update SystemNotifyMessage record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemNotifyMessageModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemNotifyMessage record by ID.
