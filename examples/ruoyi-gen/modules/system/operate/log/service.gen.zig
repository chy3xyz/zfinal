...
const SystemOperateLogModel = model.SystemOperateLogModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemOperateLog;
pub const Instance = SystemOperateLogModel.Instance;

/// List all SystemOperateLog records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemOperateLogModel.findAll(db, allocator);
}

/// Find one SystemOperateLog by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemOperateLogModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemOperateLogModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemOperateLogModel.count(db);
}

/// Create a new SystemOperateLog record. Validates input, returns instance with .id set.
...
/// Update SystemOperateLog record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemOperateLogModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemOperateLog record by ID.
