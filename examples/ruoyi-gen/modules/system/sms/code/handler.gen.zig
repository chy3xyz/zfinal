...
/// List SystemSmsCode records with pagination + rate limiting.
...
/// Create SystemSmsCode record (CSRF-protected).
...
/// Update SystemSmsCode record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("create_ip")) |v| item.data.create_ip = v;
    if (try ctx.getPara("scene")) |v| item.data.scene = std.fmt.parseInt(i64, v, 10) catch item.data.scene;
    if (try ctx.getPara("today_index")) |v| item.data.today_index = std.fmt.parseInt(i64, v, 10) catch item.data.today_index;
    if (try ctx.getPara("used")) |v| item.data.used = std.fmt.parseInt(i64, v, 10) catch item.data.used;
    if (try ctx.getPara("used_time")) |v| item.data.used_time = v;
    if (try ctx.getPara("used_ip")) |v| item.data.used_ip = v;
...
/// Delete SystemSmsCode record (CSRF-protected).
...
/// Partial update SystemSmsCode record (CSRF-protected, PATCH).
