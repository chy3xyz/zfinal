...
const SystemSmsLogModel = model.SystemSmsLogModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemSmsLog;
pub const Instance = SystemSmsLogModel.Instance;

/// List all SystemSmsLog records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemSmsLogModel.findAll(db, allocator);
}

/// Find one SystemSmsLog by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemSmsLogModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemSmsLogModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemSmsLogModel.count(db);
}

/// Create a new SystemSmsLog record. Validates input, returns instance with .id set.
...
/// Update SystemSmsLog record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemSmsLogModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemSmsLog record by ID.
