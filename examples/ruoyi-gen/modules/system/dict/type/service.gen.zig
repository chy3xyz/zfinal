...
const SystemDictTypeModel = model.SystemDictTypeModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemDictType;
pub const Instance = SystemDictTypeModel.Instance;

/// List all SystemDictType records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemDictTypeModel.findAll(db, allocator);
}

/// Find one SystemDictType by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemDictTypeModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemDictTypeModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemDictTypeModel.count(db);
}

/// Create a new SystemDictType record. Validates input, returns instance with .id set.
...
/// Update SystemDictType record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemDictTypeModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemDictType record by ID.
