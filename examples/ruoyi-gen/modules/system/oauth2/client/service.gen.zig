...
const SystemOauth2ClientModel = model.SystemOauth2ClientModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemOauth2Client;
pub const Instance = SystemOauth2ClientModel.Instance;

/// List all SystemOauth2Client records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemOauth2ClientModel.findAll(db, allocator);
}

/// Find one SystemOauth2Client by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemOauth2ClientModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemOauth2ClientModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemOauth2ClientModel.count(db);
}

/// Create a new SystemOauth2Client record. Validates input, returns instance with .id set.
...
/// Update SystemOauth2Client record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemOauth2ClientModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemOauth2Client record by ID.
