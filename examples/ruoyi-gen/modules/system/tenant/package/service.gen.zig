...
const SystemTenantPackageModel = model.SystemTenantPackageModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemTenantPackage;
pub const Instance = SystemTenantPackageModel.Instance;

/// List all SystemTenantPackage records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemTenantPackageModel.findAll(db, allocator);
}

/// Find one SystemTenantPackage by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemTenantPackageModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemTenantPackageModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemTenantPackageModel.count(db);
}

/// Create a new SystemTenantPackage record. Validates input, returns instance with .id set.
...
/// Update SystemTenantPackage record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemTenantPackageModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemTenantPackage record by ID.
