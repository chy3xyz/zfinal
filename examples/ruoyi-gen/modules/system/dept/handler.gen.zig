...
/// List SystemDept records with pagination + rate limiting.
...
/// Create SystemDept record (CSRF-protected).
pub fn create(ctx: *zfinal.Context) !void {
    ...
    const data: service.Data = .{
        ...
        .leader_user_id = std.fmt.parseInt(i64, (try ctx.getPara("leader_user_id")) orelse "0", 10) catch 0,
        .phone = (try ctx.getPara("phone")) orelse null,
...
/// Update SystemDept record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("leader_user_id")) |v| item.data.leader_user_id = std.fmt.parseInt(i64, v, 10) catch item.data.leader_user_id;
    if (try ctx.getPara("phone")) |v| item.data.phone = v;
...
/// Delete SystemDept record (CSRF-protected).
...
/// Partial update SystemDept record (CSRF-protected, PATCH).
