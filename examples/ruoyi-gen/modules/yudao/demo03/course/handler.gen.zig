...
/// List YudaoDemo03Course records with pagination + rate limiting.
...
/// Create YudaoDemo03Course record (CSRF-protected).
pub fn create(ctx: *zfinal.Context) !void {
    try csrfGuard(ctx);
    const db = try pool_ref.acquire();
    defer pool_ref.release(db) catch {};
    const data: service.Data = .{
        .student_id = std.fmt.parseInt(i64, (try ctx.getPara("student_id")) orelse "0", 10) catch 0,
        .name = (try ctx.getPara("name")) orelse "",
        .score = std.fmt.parseInt(i64, (try ctx.getPara("score")) orelse "0", 10) catch 0,
...
/// Update YudaoDemo03Course record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("student_id")) |v| item.data.student_id = std.fmt.parseInt(i64, v, 10) catch item.data.student_id;
    if (try ctx.getPara("name")) |v| item.data.name = v;
    if (try ctx.getPara("score")) |v| item.data.score = std.fmt.parseInt(i64, v, 10) catch item.data.score;
...
/// Delete YudaoDemo03Course record (CSRF-protected).
...
/// Partial update YudaoDemo03Course record (CSRF-protected, PATCH).
