...
const YudaoDemo03StudentModel = model.YudaoDemo03StudentModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.YudaoDemo03Student;
pub const Instance = YudaoDemo03StudentModel.Instance;

/// List all YudaoDemo03Student records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return YudaoDemo03StudentModel.findAll(db, allocator);
}

/// Find one YudaoDemo03Student by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return YudaoDemo03StudentModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return YudaoDemo03StudentModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return YudaoDemo03StudentModel.count(db);
}

/// Create a new YudaoDemo03Student record. Validates input, returns instance with .id set.
...
/// Update YudaoDemo03Student record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try YudaoDemo03StudentModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete YudaoDemo03Student record by ID.
