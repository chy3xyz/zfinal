...
const YudaoDemo03GradeModel = model.YudaoDemo03GradeModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.YudaoDemo03Grade;
pub const Instance = YudaoDemo03GradeModel.Instance;

/// List all YudaoDemo03Grade records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return YudaoDemo03GradeModel.findAll(db, allocator);
}

/// Find one YudaoDemo03Grade by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return YudaoDemo03GradeModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return YudaoDemo03GradeModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return YudaoDemo03GradeModel.count(db);
}

/// Create a new YudaoDemo03Grade record. Validates input, returns instance with .id set.
...
/// Update YudaoDemo03Grade record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try YudaoDemo03GradeModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete YudaoDemo03Grade record by ID.
