...
const InfraJobLogModel = model.InfraJobLogModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.InfraJobLog;
pub const Instance = InfraJobLogModel.Instance;

/// List all InfraJobLog records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return InfraJobLogModel.findAll(db, allocator);
}

/// Find one InfraJobLog by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return InfraJobLogModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return InfraJobLogModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return InfraJobLogModel.count(db);
}

/// Create a new InfraJobLog record. Validates input, returns instance with .id set.
...
/// Update InfraJobLog record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try InfraJobLogModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete InfraJobLog record by ID.
