...
const InfraJobModel = model.InfraJobModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.InfraJob;
pub const Instance = InfraJobModel.Instance;

/// List all InfraJob records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return InfraJobModel.findAll(db, allocator);
}

/// Find one InfraJob by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return InfraJobModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return InfraJobModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return InfraJobModel.count(db);
}

/// Create a new InfraJob record. Validates input, returns instance with .id set.
...
/// Update InfraJob record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try InfraJobModel.findById(db, id, db.allocator) orelse return error.NotFound;
