...
/// List InfraJob records with pagination + rate limiting.
...
/// Create InfraJob record (CSRF-protected).
...
/// Update InfraJob record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("cron_expression")) |v| item.data.cron_expression = v;
    if (try ctx.getPara("retry_count")) |v| item.data.retry_count = std.fmt.parseInt(i64, v, 10) catch item.data.retry_count;
    if (try ctx.getPara("retry_interval")) |v| item.data.retry_interval = std.fmt.parseInt(i64, v, 10) catch item.data.retry_interval;
    if (try ctx.getPara("monitor_timeout")) |v| item.data.monitor_timeout = std.fmt.parseInt(i64, v, 10) catch item.data.monitor_timeout;
...
/// Delete InfraJob record (CSRF-protected).
...
/// Partial update InfraJob record (CSRF-protected, PATCH).
