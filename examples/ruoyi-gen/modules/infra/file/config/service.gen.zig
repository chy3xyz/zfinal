...
const InfraFileConfigModel = model.InfraFileConfigModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.InfraFileConfig;
pub const Instance = InfraFileConfigModel.Instance;

/// List all InfraFileConfig records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return InfraFileConfigModel.findAll(db, allocator);
}

/// Find one InfraFileConfig by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return InfraFileConfigModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return InfraFileConfigModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return InfraFileConfigModel.count(db);
}

/// Create a new InfraFileConfig record. Validates input, returns instance with .id set.
...
/// Update InfraFileConfig record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try InfraFileConfigModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete InfraFileConfig record by ID.
