...
const InfraFileModel = model.InfraFileModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.InfraFile;
pub const Instance = InfraFileModel.Instance;

/// List all InfraFile records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return InfraFileModel.findAll(db, allocator);
}

/// Find one InfraFile by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return InfraFileModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return InfraFileModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return InfraFileModel.count(db);
}

/// Create a new InfraFile record. Validates input, returns instance with .id set.
...
/// Update InfraFile record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try InfraFileModel.findById(db, id, db.allocator) orelse return error.NotFound;
