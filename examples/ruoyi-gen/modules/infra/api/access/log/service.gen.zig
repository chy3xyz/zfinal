...
const InfraApiAccessLogModel = model.InfraApiAccessLogModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.InfraApiAccessLog;
pub const Instance = InfraApiAccessLogModel.Instance;

/// List all InfraApiAccessLog records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return InfraApiAccessLogModel.findAll(db, allocator);
}

/// Find one InfraApiAccessLog by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return InfraApiAccessLogModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return InfraApiAccessLogModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return InfraApiAccessLogModel.count(db);
}

/// Create a new InfraApiAccessLog record. Validates input, returns instance with .id set.
...
/// Update InfraApiAccessLog record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try InfraApiAccessLogModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete InfraApiAccessLog record by ID.
