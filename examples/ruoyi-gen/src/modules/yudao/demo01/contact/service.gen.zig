...
const YudaoDemo01ContactModel = model.YudaoDemo01ContactModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.YudaoDemo01Contact;
pub const Instance = YudaoDemo01ContactModel.Instance;

/// List all YudaoDemo01Contact records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return YudaoDemo01ContactModel.findAll(db, allocator);
}

/// Find one YudaoDemo01Contact by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return YudaoDemo01ContactModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return YudaoDemo01ContactModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return YudaoDemo01ContactModel.count(db);
}

/// Create a new YudaoDemo01Contact record. Validates input, returns instance with .id set.
...
/// Update YudaoDemo01Contact record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try YudaoDemo01ContactModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete YudaoDemo01Contact record by ID.
