...
/// List InfraFileConfig records with pagination + rate limiting.
...
/// Create InfraFileConfig record (CSRF-protected).
pub fn create(ctx: *zfinal.Context) !void {
    ...
    const data: service.Data = .{
        ...
        .storage = std.fmt.parseInt(i64, (try ctx.getPara("storage")) orelse "0", 10) catch 0,
        .remark = (try ctx.getPara("remark")) orelse null,
        .master = if ((try ctx.getPara("master"))) |v| (std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1") or std.mem.eql(u8, v, "t")) else false,
        .config = (try ctx.getPara("config")) orelse "",
...
/// Update InfraFileConfig record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("storage")) |v| item.data.storage = std.fmt.parseInt(i64, v, 10) catch item.data.storage;
    if (try ctx.getPara("remark")) |v| item.data.remark = v;
    if (try ctx.getPara("master")) |v| item.data.master = std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1");
    if (try ctx.getPara("config")) |v| item.data.config = v;
...
/// Delete InfraFileConfig record (CSRF-protected).
...
/// Partial update InfraFileConfig record (CSRF-protected, PATCH).
