...
/// List InfraApiAccessLog records with pagination + rate limiting.
...
/// Create InfraApiAccessLog record (CSRF-protected).
pub fn create(ctx: *zfinal.Context) !void {
    ...
    const data: service.Data = .{
        ...
        .request_params = (try ctx.getPara("request_params")) orelse null,
        .response_body = (try ctx.getPara("response_body")) orelse null,
        .user_ip = (try ctx.getPara("user_ip")) orelse "",
        .user_agent = (try ctx.getPara("user_agent")) orelse "",
        .operate_module = (try ctx.getPara("operate_module")) orelse null,
        .operate_name = (try ctx.getPara("operate_name")) orelse null,
        .operate_type = std.fmt.parseInt(i64, (try ctx.getPara("operate_type")) orelse "0", 10) catch 0,
        .begin_time = (try ctx.getPara("begin_time")) orelse "",
        .end_time = (try ctx.getPara("end_time")) orelse "",
        .duration = std.fmt.parseInt(i64, (try ctx.getPara("duration")) orelse "0", 10) catch 0,
        .result_code = std.fmt.parseInt(i64, (try ctx.getPara("result_code")) orelse "0", 10) catch 0,
        .result_msg = (try ctx.getPara("result_msg")) orelse null,
...
/// Update InfraApiAccessLog record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("response_body")) |v| item.data.response_body = v;
    if (try ctx.getPara("user_ip")) |v| item.data.user_ip = v;
    if (try ctx.getPara("user_agent")) |v| item.data.user_agent = v;
    if (try ctx.getPara("operate_module")) |v| item.data.operate_module = v;
    if (try ctx.getPara("operate_name")) |v| item.data.operate_name = v;
    if (try ctx.getPara("operate_type")) |v| item.data.operate_type = std.fmt.parseInt(i64, v, 10) catch item.data.operate_type;
    ...
    if (try ctx.getPara("result_code")) |v| item.data.result_code = std.fmt.parseInt(i64, v, 10) catch item.data.result_code;
    if (try ctx.getPara("result_msg")) |v| item.data.result_msg = v;
...
/// Delete InfraApiAccessLog record (CSRF-protected).
...
/// Partial update InfraApiAccessLog record (CSRF-protected, PATCH).
