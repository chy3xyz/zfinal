...
const InfraCodegenColumnModel = model.InfraCodegenColumnModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.InfraCodegenColumn;
pub const Instance = InfraCodegenColumnModel.Instance;

/// List all InfraCodegenColumn records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return InfraCodegenColumnModel.findAll(db, allocator);
}

/// Find one InfraCodegenColumn by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return InfraCodegenColumnModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return InfraCodegenColumnModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return InfraCodegenColumnModel.count(db);
}

/// Create a new InfraCodegenColumn record. Validates input, returns instance with .id set.
...
/// Update InfraCodegenColumn record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try InfraCodegenColumnModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete InfraCodegenColumn record by ID.
