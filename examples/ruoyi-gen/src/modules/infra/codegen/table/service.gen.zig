...
const InfraCodegenTableModel = model.InfraCodegenTableModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.InfraCodegenTable;
pub const Instance = InfraCodegenTableModel.Instance;

/// List all InfraCodegenTable records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return InfraCodegenTableModel.findAll(db, allocator);
}

/// Find one InfraCodegenTable by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return InfraCodegenTableModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return InfraCodegenTableModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return InfraCodegenTableModel.count(db);
}

/// Create a new InfraCodegenTable record. Validates input, returns instance with .id set.
...
/// Update InfraCodegenTable record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try InfraCodegenTableModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete InfraCodegenTable record by ID.
