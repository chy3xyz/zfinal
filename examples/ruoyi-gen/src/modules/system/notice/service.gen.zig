...
const SystemNoticeModel = model.SystemNoticeModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemNotice;
pub const Instance = SystemNoticeModel.Instance;

/// List all SystemNotice records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemNoticeModel.findAll(db, allocator);
}

/// Find one SystemNotice by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemNoticeModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemNoticeModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemNoticeModel.count(db);
}

/// Create a new SystemNotice record. Validates input, returns instance with .id set.
...
/// Update SystemNotice record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemNoticeModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemNotice record by ID.
