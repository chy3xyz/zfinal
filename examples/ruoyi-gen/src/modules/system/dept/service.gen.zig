...
const SystemDeptModel = model.SystemDeptModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemDept;
pub const Instance = SystemDeptModel.Instance;

/// List all SystemDept records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemDeptModel.findAll(db, allocator);
}

/// Find one SystemDept by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemDeptModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemDeptModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemDeptModel.count(db);
}

/// Create a new SystemDept record. Validates input, returns instance with .id set.
...
/// Update SystemDept record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemDeptModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemDept record by ID.
