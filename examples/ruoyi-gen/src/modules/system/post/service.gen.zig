...
const SystemPostModel = model.SystemPostModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemPost;
pub const Instance = SystemPostModel.Instance;

/// List all SystemPost records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemPostModel.findAll(db, allocator);
}

/// Find one SystemPost by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemPostModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemPostModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemPostModel.count(db);
}

/// Create a new SystemPost record. Validates input, returns instance with .id set.
...
/// Update SystemPost record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemPostModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemPost record by ID.
