...
/// List SystemDictType records with pagination + rate limiting.
...
/// Create SystemDictType record (CSRF-protected).
pub fn create(ctx: *zfinal.Context) !void {
    ...
    const data: service.Data = .{
        ...
        .deleted_time = (try ctx.getPara("deleted_time")) orelse null,
    };
...
/// Update SystemDictType record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("deleted_time")) |v| item.data.deleted_time = v;
    // Validation handled by service layer
    try item.save(db);
    try ctx.renderJson(.{ .ok = true });
}

/// Delete SystemDictType record (CSRF-protected).
...
/// Partial update SystemDictType record (CSRF-protected, PATCH).
