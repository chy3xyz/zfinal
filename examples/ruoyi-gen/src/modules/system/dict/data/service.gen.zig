...
const SystemDictDataModel = model.SystemDictDataModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemDictData;
pub const Instance = SystemDictDataModel.Instance;

/// List all SystemDictData records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemDictDataModel.findAll(db, allocator);
}

/// Find one SystemDictData by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemDictDataModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemDictDataModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemDictDataModel.count(db);
}

/// Create a new SystemDictData record. Validates input, returns instance with .id set.
...
/// Update SystemDictData record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemDictDataModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemDictData record by ID.
