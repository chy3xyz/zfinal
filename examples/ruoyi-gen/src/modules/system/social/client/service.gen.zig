...
const SystemSocialClientModel = model.SystemSocialClientModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemSocialClient;
pub const Instance = SystemSocialClientModel.Instance;

/// List all SystemSocialClient records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemSocialClientModel.findAll(db, allocator);
}

/// Find one SystemSocialClient by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemSocialClientModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemSocialClientModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemSocialClientModel.count(db);
}

/// Create a new SystemSocialClient record. Validates input, returns instance with .id set.
...
/// Update SystemSocialClient record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemSocialClientModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemSocialClient record by ID.
