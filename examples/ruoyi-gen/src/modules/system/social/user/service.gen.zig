...
const SystemSocialUserModel = model.SystemSocialUserModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemSocialUser;
pub const Instance = SystemSocialUserModel.Instance;

/// List all SystemSocialUser records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemSocialUserModel.findAll(db, allocator);
}

/// Find one SystemSocialUser by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemSocialUserModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemSocialUserModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemSocialUserModel.count(db);
}

/// Create a new SystemSocialUser record. Validates input, returns instance with .id set.
...
/// Update SystemSocialUser record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemSocialUserModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemSocialUser record by ID.
