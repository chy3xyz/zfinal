...
const SystemRoleModel = model.SystemRoleModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemRole;
pub const Instance = SystemRoleModel.Instance;

/// List all SystemRole records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemRoleModel.findAll(db, allocator);
}

/// Find one SystemRole by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemRoleModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemRoleModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemRoleModel.count(db);
}

/// Create a new SystemRole record. Validates input, returns instance with .id set.
...
/// Update SystemRole record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemRoleModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemRole record by ID.
