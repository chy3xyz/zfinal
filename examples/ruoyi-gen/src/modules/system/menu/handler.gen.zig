...
/// List SystemMenu records with pagination + rate limiting.
...
/// Create SystemMenu record (CSRF-protected).
pub fn create(ctx: *zfinal.Context) !void {
    ...
    const data: service.Data = .{
        ...
        .permission = (try ctx.getPara("permission")) orelse "",
        ...
        .parent_id = std.fmt.parseInt(i64, (try ctx.getPara("parent_id")) orelse "0", 10) catch 0,
        .path = (try ctx.getPara("path")) orelse null,
        .icon = (try ctx.getPara("icon")) orelse null,
        .component = (try ctx.getPara("component")) orelse null,
        .component_name = (try ctx.getPara("component_name")) orelse null,
        ...
        .keep_alive = if ((try ctx.getPara("keep_alive"))) |v| (std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1") or std.mem.eql(u8, v, "t")) else false,
        .always_show = if ((try ctx.getPara("always_show"))) |v| (std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1") or std.mem.eql(u8, v, "t")) else false,
...
/// Update SystemMenu record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("permission")) |v| item.data.permission = v;
    ...
    if (try ctx.getPara("parent_id")) |v| item.data.parent_id = std.fmt.parseInt(i64, v, 10) catch item.data.parent_id;
    if (try ctx.getPara("path")) |v| item.data.path = v;
    if (try ctx.getPara("icon")) |v| item.data.icon = v;
    if (try ctx.getPara("component")) |v| item.data.component = v;
    if (try ctx.getPara("component_name")) |v| item.data.component_name = v;
    ...
    if (try ctx.getPara("keep_alive")) |v| item.data.keep_alive = std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1");
    if (try ctx.getPara("always_show")) |v| item.data.always_show = std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1");
...
/// Delete SystemMenu record (CSRF-protected).
...
/// Partial update SystemMenu record (CSRF-protected, PATCH).
