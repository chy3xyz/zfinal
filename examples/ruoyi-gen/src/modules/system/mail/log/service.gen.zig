...
const SystemMailLogModel = model.SystemMailLogModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemMailLog;
pub const Instance = SystemMailLogModel.Instance;

/// List all SystemMailLog records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemMailLogModel.findAll(db, allocator);
}

/// Find one SystemMailLog by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemMailLogModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemMailLogModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemMailLogModel.count(db);
}

/// Create a new SystemMailLog record. Validates input, returns instance with .id set.
...
/// Update SystemMailLog record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemMailLogModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemMailLog record by ID.
