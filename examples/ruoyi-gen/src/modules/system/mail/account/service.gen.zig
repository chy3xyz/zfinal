...
const SystemMailAccountModel = model.SystemMailAccountModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemMailAccount;
pub const Instance = SystemMailAccountModel.Instance;

/// List all SystemMailAccount records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemMailAccountModel.findAll(db, allocator);
}

/// Find one SystemMailAccount by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemMailAccountModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemMailAccountModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemMailAccountModel.count(db);
}

/// Create a new SystemMailAccount record. Validates input, returns instance with .id set.
...
/// Update SystemMailAccount record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemMailAccountModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemMailAccount record by ID.
