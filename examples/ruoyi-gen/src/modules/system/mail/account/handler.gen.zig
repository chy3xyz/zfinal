...
/// List SystemMailAccount records with pagination + rate limiting.
...
/// Create SystemMailAccount record (CSRF-protected).
...
/// Update SystemMailAccount record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("mail")) |v| item.data.mail = v;
    if (try ctx.getPara("username")) |v| item.data.username = v;
    if (try ctx.getPara("password")) |v| item.data.password = v;
    if (try ctx.getPara("host")) |v| item.data.host = v;
    if (try ctx.getPara("port")) |v| item.data.port = std.fmt.parseInt(i64, v, 10) catch item.data.port;
    if (try ctx.getPara("ssl_enable")) |v| item.data.ssl_enable = std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1");
    if (try ctx.getPara("starttls_enable")) |v| item.data.starttls_enable = std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1");
...
/// Delete SystemMailAccount record (CSRF-protected).
...
/// Partial update SystemMailAccount record (CSRF-protected, PATCH).
