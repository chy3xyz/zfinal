...
const SystemOauth2CodeModel = model.SystemOauth2CodeModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemOauth2Code;
pub const Instance = SystemOauth2CodeModel.Instance;

/// List all SystemOauth2Code records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemOauth2CodeModel.findAll(db, allocator);
}

/// Find one SystemOauth2Code by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemOauth2CodeModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemOauth2CodeModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemOauth2CodeModel.count(db);
}

/// Create a new SystemOauth2Code record. Validates input, returns instance with .id set.
...
/// Update SystemOauth2Code record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemOauth2CodeModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemOauth2Code record by ID.
