...
/// SystemOauth2Code model — maps to `system_oauth2_code` table.
pub const SystemOauth2Code = struct {
    id: ?i64 = null,
    user_id: i64,
    user_type: i64,
    code: []const u8,
...
pub const SystemOauth2CodeModel = zfinal.Model(SystemOauth2Code, "system_oauth2_code");
...
pub const fieldMap = [_]struct { db: []const u8, json: []const u8 }{
        ...
        .{ .db = "redirect_uri", .json = "redirect_uri" },
        .{ .db = "state", .json = "state" },
...
/// Validate SystemOauth2Code data before insert/update.
pub fn validate(data: SystemOauth2Code) !void {
    ...
    if (data.state.len == 0) return error.ValidationError;
