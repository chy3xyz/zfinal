...
/// SystemOauth2AccessToken model — maps to `system_oauth2_access_token` table.
pub const SystemOauth2AccessToken = struct {
    id: ?i64 = null,
    user_id: i64,
    user_type: i64,
    user_info: []const u8,
...
pub const SystemOauth2AccessTokenModel = zfinal.Model(SystemOauth2AccessToken, "system_oauth2_access_token");
...
/// Validate SystemOauth2AccessToken data before insert/update.
pub fn validate(data: SystemOauth2AccessToken) !void {
    if (data.user_info.len == 0) return error.ValidationError;
    if (data.access_token.len == 0) return error.ValidationError;
