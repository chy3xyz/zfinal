...
/// List SystemOauth2AccessToken records with pagination + rate limiting.
...
/// Show SystemOauth2AccessToken by ID.
...
/// Create SystemOauth2AccessToken record (CSRF-protected).
pub fn create(ctx: *zfinal.Context) !void {
    ...
    const data: service.Data = .{
        ...
        .user_info = (try ctx.getPara("user_info")) orelse "",
        .access_token = (try ctx.getPara("access_token")) orelse "",
...
/// Update SystemOauth2AccessToken record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("user_info")) |v| item.data.user_info = v;
    if (try ctx.getPara("access_token")) |v| item.data.access_token = v;
...
/// Delete SystemOauth2AccessToken record (CSRF-protected).
...
/// Partial update SystemOauth2AccessToken record (CSRF-protected, PATCH).
