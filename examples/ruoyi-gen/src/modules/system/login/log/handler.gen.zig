...
/// List SystemLoginLog records with pagination + rate limiting.
...
/// Create SystemLoginLog record (CSRF-protected).
pub fn create(ctx: *zfinal.Context) !void {
    try csrfGuard(ctx);
    const db = try pool.acquire();
    defer pool.release(db) catch {};
    const data: service.Data = .{
        .log_type = std.fmt.parseInt(i64, (try ctx.getPara("log_type")) orelse "0", 10) catch 0,
        ...
        .result = std.fmt.parseInt(i64, (try ctx.getPara("result")) orelse "0", 10) catch 0,
...
/// Update SystemLoginLog record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("log_type")) |v| item.data.log_type = std.fmt.parseInt(i64, v, 10) catch item.data.log_type;
    ...
    if (try ctx.getPara("result")) |v| item.data.result = std.fmt.parseInt(i64, v, 10) catch item.data.result;
...
/// Delete SystemLoginLog record (CSRF-protected).
...
/// Partial update SystemLoginLog record (CSRF-protected, PATCH).
