...
/// List SystemUserRole records with pagination + rate limiting.
...
/// Create SystemUserRole record (CSRF-protected).
pub fn create(ctx: *zfinal.Context) !void {
    ...
    const data: service.Data = .{
            ...
            .role_id = std.fmt.parseInt(i64, (try ctx.getPara("role_id")) orelse "0", 10) catch 0,
            .creator = (try ctx.getPara("creator")) orelse null,
            .create_time = (try ctx.getPara("create_time")) orelse null,
            .updater = (try ctx.getPara("updater")) orelse null,
            .update_time = (try ctx.getPara("update_time")) orelse null,
            .deleted = if ((try ctx.getPara("deleted"))) |v| (std.mem.eql(u8, v, "true") or std.mem.eql(u8, v, "1") or std.mem.eql(u8, v, "t")) else null,
...
/// Update SystemUserRole record (CSRF-protected).
pub fn update(ctx: *zfinal.Context) !void {
    ...
    if (try ctx.getPara("role_id")) |v| item.data.role_id = std.fmt.parseInt(i64, v, 10) catch item.data.role_id;
...
/// Delete SystemUserRole record (CSRF-protected).
...
/// Partial update SystemUserRole record (CSRF-protected, PATCH).
