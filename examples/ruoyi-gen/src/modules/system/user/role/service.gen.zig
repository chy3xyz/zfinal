...
const SystemUserRoleModel = model.SystemUserRoleModel;
const validate = model.validate;

// Re-export types so handler doesn't need to import model directly.
pub const Data = model.SystemUserRole;
pub const Instance = SystemUserRoleModel.Instance;

/// List all SystemUserRole records.
pub fn findAll(db: *zfinal.DB, allocator: std.mem.Allocator) ![]Data {
    return SystemUserRoleModel.findAll(db, allocator);
}

/// Find one SystemUserRole by primary key.
pub fn findById(db: *zfinal.DB, id: i64, allocator: std.mem.Allocator) !?Instance {
    return SystemUserRoleModel.findById(db, id, allocator);
}

/// Paginated list.
pub fn paginate(db: *zfinal.DB, page: u32, size: u32, allocator: std.mem.Allocator) ![]Instance {
    return SystemUserRoleModel.paginate(db, page, size, allocator);
}

/// Total row count.
pub fn count(db: *zfinal.DB) !i64 {
    return SystemUserRoleModel.count(db);
}

/// Create a new SystemUserRole record. Validates input, returns instance with .id set.
...
/// Update SystemUserRole record by ID. Returns updated instance.
pub fn update(db: *zfinal.DB, id: i64, data: Data) !Instance {
    var item = try SystemUserRoleModel.findById(db, id, db.allocator) orelse return error.NotFound;
...
/// Delete SystemUserRole record by ID.
