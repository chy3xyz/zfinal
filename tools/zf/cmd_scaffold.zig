//! Scaffold / codegen helpers: `zf new`, `zf g`, docker, deploy, life, upgrade, ai.
...
pub fn createProject(allocator: std.mem.Allocator, project_name: []const u8, clean: bool, json_mode: bool) !void {
    ...
    if (json_mode) {
        ...
        try out_buf.appendSlice(allocator, "  \"generator\": \"zf new\",\n");
        try out_buf.appendSlice(allocator, "  \"version\": \"");
        try out_buf.appendSlice(allocator, zf_cfg.semver);
        try out_buf.appendSlice(allocator, "\",\n");
        try out_buf.appendSlice(allocator, "  \"project\": \"");
        try appendJsonString(allocator, &out_buf, project_name);
        try out_buf.appendSlice(allocator, "\",\n");
        try out_buf.appendSlice(allocator, "  \"clean\": ");
        try out_buf.appendSlice(allocator, if (clean) "true" else "false");
        try out_buf.appendSlice(allocator, ",\n");
        try out_buf.appendSlice(allocator, "  \"next_steps\": [\n");
        try out_buf.appendSlice(allocator, "    \"cd ");
        try appendJsonString(allocator, &out_buf, project_name);
        try out_buf.appendSlice(allocator, "\",\n");
        try out_buf.appendSlice(allocator, "    \"zf crud:sql schema.sql --json\",\n");
        try out_buf.appendSlice(allocator, "    \"zf routes --json\",\n");
        try out_buf.appendSlice(allocator, "    \"zf check && zig build test\"\n");
        try out_buf.appendSlice(allocator, "  ]\n");
        try out_buf.appendSlice(allocator, "}\n");
...
pub fn generateDocker(_: std.mem.Allocator) !void {
...
pub fn handleDeploy(allocator: std.mem.Allocator) !void {
...
pub fn generatePlugin(allocator: std.mem.Allocator, name: []const u8) !void {
...
pub fn handleLife(allocator: std.mem.Allocator, sub: []const u8, _: []const u8) !void {
...
pub fn handleUpgrade(allocator: std.mem.Allocator) !void {
...
pub fn writeDeploymentFiles(allocator: std.mem.Allocator, cwd: std.Io.Dir, project_name: []const u8) !void {
